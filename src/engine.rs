...
fn prepare_input() -> Result<UnboundedReceiver<KeyPress>> {
    ...
    let onekeydown = browser::closure_wrap(Box::new(move |keycode: web_sys::KeyboardEvent| {
        keydown_sender
            .borrow_mut()
            .start_send(KeyPress::KeyDown(keycode));
    }) as Box<dyn FnMut(web_sys::KeyboardEvent)>);

    let onekeyup = browser::closure_wrap(Box::new(move |keycode: web_sys::KeyboardEvent| {
        keyup_sender
            .borrow_mut()
            .start_send(KeyPress::KeyUp(keycode));
    }) as Box<dyn FnMut(web_sys::KeyboardEvent)>);

    browser::window()?.set_onkeydown(Some(onekeydown.as_ref().unchecked_ref()));

    browser::window()?.set_onkeyup(Some(onekeyup.as_ref().unchecked_ref()));
    ...
    loop {
        match keyevent_receiver.try_next() {
            ...
            Ok(Some(evt)) => match evt {
                KeyPress::KeyUp(evt) => state.set_releaded(&evt.code()),
